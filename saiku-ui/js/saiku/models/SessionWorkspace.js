...
var SessionWorkspace = Backbone.Model.extend({
                ...
                if (filePath.indexOf('.saiku') === filePath.length - 6) {
                    ...
                    if (Saiku.Dashboards) {
                        ...
                        if (!Settings.INITIAL_QUERY && paramsURI.splash) {
                            Saiku.tabs.add(new SchemaConfig());
                        }
    ...
    process_datasources: function(model, response) {
		globalSessionWorkspace = this;
		globalSessionWorkspace.ready = false;
        ...
        if (!this.initialized) {
			...
			globalSessionWorkspace.ready = true;
        }
        else {
			globalSessionWorkspace.ready = true;
            if (!Settings.INITIAL_QUERY) {
	...
	url: function() {
		if (this.first) {
			this.first = false;
			return encodeURI(Saiku.session.username + "/discover/"+Saiku.session.sessionid);
		}
		else {
			return encodeURI(Saiku.session.username + "/discover/"+Saiku.session.sessionid+"/refresh");
		}
	}
});
