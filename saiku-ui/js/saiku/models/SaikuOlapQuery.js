...
SaikuOlapQueryHelper.prototype.clearAxis = function(axisName,i) {
	var axis = this.model().queryModel.axes[axisName].hierarchies;
	if (!i) {
		this.model().queryModel.axes[axisName].hierarchies = [];
	}else{
		// axis =axis.splice(0,i-1)+axis.splice(i);
		delete  axis[(i-1)];
		axis = axis.filter(function(item){return item});
	}
};
