...
var Query = Backbone.Model.extend({
    ...
    setProperty: function(key, value) {
		...
		if (key==="saiku.olap.query.automatic_execution") {
			var autoUpdate = $('#automatic_icon');
			autoUpdate.find('input')[0].checked = !value;
		}
    },
        ...
        if (exModel.queryType == "OLAP") {
            if (exModel.type == "QUERYMODEL") {
                ...
                if (!columnsOk && !detailsOk) {
                    errorMessage += '<span class="i18n">Вам нужно включить хотя бы один показатель или уровень в столбцы для допустимого запроса. Вам нужно включить хотя бы один уровень в строки для допустимого запроса.</span>';
                }
                if(!rowsOk) {
                    errorMessage += '<span class="i18n">Вам нужно включить хотя бы один уровень в строки для допустимого запроса.</span>';
