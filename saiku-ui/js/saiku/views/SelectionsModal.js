...
var SelectionsModal = Modal.extend({
    ...
    initialize: function(args) {
        ...
        var self = this;
        _.extend(this, args);
        this.options.title = "<span class='i18n'>НАСТРОЙКА ОТОБРАЖЕНИЯ И ФИЛЬТРАЦИИ ДЛЯ АНАЛИТИЧЕСКОГО РАЗРЕЗА</span> " + this.name;
...
//        showTotalsEl.removeAttr("disabled");
    ...
    populate: function(model, response) {
        ...
        $(this.el).find('.filterbox').autocomplete({
            ...
            select:  function(event, ui) {
                ...
                var option = '<li class="option_value" title="label"><input type="checkbox" class="check_option" value="'
    ...
    post_render: function(args) {
        ...
        var parent =$(args.modal.el).parents('.ui-dialog');
		parent.css({ width: width, left: "inherit", margin:"0", height: 630 })
			.offset({ left: left});
		parent[0].classList.add("filter_modal_in_analysys");//Помечаем данное окно для стилизации

		// $(args.modal.el).parents('.ui-dialog')
         //    .css({ width: width, left: "inherit", margin:"0", height: 630 })
         //    .offset({ left: left});
        //
		// this.el.classList.add("filter_modal_in_analysys");//Помечаем данное окно для стилизации
    ...
    move_selection: function(event) {
        ...
        if (!action){
			action = $(event.target.parentElement).attr('id');
		}
