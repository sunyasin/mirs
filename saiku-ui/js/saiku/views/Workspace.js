...
var Workspace = Backbone.View.extend({
    ...
    events: {
        ...
        'click .cancel' : 'cancel',
        'click .admin' : 'admin',
		'click .clear_dimension' : 'remove_dimension',
		'click .clear_measure' : 'remove_measure',
		...
		'click  #addMeasureButton': 'addMeasures',
		'click  #cancelAddMeasureButton': 'cancelAddMeasures'


    },
	cancelAddMeasures:function () {
		$(".add_measure_modal_dialog").addClass("hide");
	},
	addMeasures: function(event){
		event.preventDefault();

		var ids = [];

		$(".add_measure_modal_dialog .select_indicator_row__div.active").find("input").each(function (index, value) {
			ids.push($(value).attr('fact-id'));
		})
		...
		DimensionListGlobal.addMeasure(ids);
	},
	recheckRow: function (event) {
		...
		if (SchemaConfigGlobal.dimensionData[0].factGroups !== null && SchemaConfigGlobal.dimensionData[parentIndex]) {
			SchemaConfigGlobal.dimensionData[parentIndex].factGroups[event.target.children[0].getAttribute("index")].facts.forEach(function (fact) {
				fact.isSelected = curStatus;
			});
		} else {
			SchemaConfigGlobal.dimensionData[event.target.children[0].getAttribute("index")].facts.forEach(function (fact) {
				fact.isSelected = curStatus;
			});
		}
	...
	recheckIndicator: function (event) {
		try {
			...
			if (SchemaConfigGlobal.dimensionData[parentIndex].factGroups !== null &&
				SchemaConfigGlobal.dimensionData[parentIndex].factGroups[measureIndex]) {
				SchemaConfigGlobal.dimensionData[parentIndex].factGroups[measureIndex].facts[factIndex].isSelected = curStatus;
			} else {
				if (SchemaConfigGlobal.dimensionData[measureIndex].facts) {
					SchemaConfigGlobal.dimensionData[measureIndex].facts[factIndex].isSelected = curStatus;
				}
			}
	...
	expandIndicatorRow: function (event) {
		...
		var shownCount = $('.indicators_group_row.active').find('.indicator_row:not(".hide")').length;
	...
	clearAll: function (event) {
		...
		SchemaConfigGlobal.dimensionDataForAdding.forEach(function (group) {
	...
	updateShowCount:function () {
		var shownCount = $('.indicators_group_row.active').find('.indicator_row:not(".hide")').length,
			allCount = $('.indicators_group_row').find('.indicator_row').length;
		var span = document.createElement("span");
		span.innerHTML = "Показано ";
		var showCountElement = $('#indicators_shown_count')[0];
		showCountElement.children = [];
   ...
   setDefaultFilters: function(filters, query){
    ...
    create_new_query: function(obj){
        ...
        // obj.selected_cube = "foms/foms/foms/%D0%A2%D0%B5%D1%81%D1%82";
        ...
        if (this.query) {
            if(Settings.QUERY_OVERWRITE_WARNING) {
                (new WarningModal({
                    ...
                    title: "Новый запрос", message: "Очистить текущий запрос?",
...
*/
	...
	remove_measure: function(event, ui) {
		if (this.query.model.type == "QUERYMODEL") {
			this.drop_zones.remove_measure(event, ui);
		}
	},
    ...
    no_results: function(args) {
        this.processing.html("По выбранным условиям данных нет").show();
    },
