...
var WorkspaceDropZone = Backbone.View.extend({
    ...
    set_measures: function(event, ui) {
        ...
        $(this.el).find('.fields_list_body.details ul.connectable li.d_measure').each( function(index, element) {
            var measure = {
				...
				"caption":$(element).find('a')[0].innerHTML
            };
    ...
    remove_dimension: function(event, ui) {
		if (ui===undefined) {
			ui = {};
			ui.helper = event.target.parentElement
		}
        if ($(ui.helper).hasClass('d_measure')) {
            ...
            this.workspace.query.helper.removeLevel(hierarchy, level);
            this.workspace.sync_query();
            this.workspace.query.run();
        }
    },

	remove_measure: function (event, ui) {
    	if ($(".measure_tree a").length < 2) {
    		return;
		}
    	var parentThis = this;
		event.preventDefault();
		var deleteMeasure = $(event.target.parentElement).find("a").attr("measure");
		$("[measure='"+deleteMeasure+"']").parent().remove();
		...
		ids=ids.join(",");
		Loader.on();
		$.get("saiku/rest/api/dimensions/ids/"+ids).then(function (res) {
			$(".d_hierarchy").each(function (index, elem) {
				var hierarchy = $(elem).attr("hierarchy");
				var dimId = parseInt(hierarchy.split('].[')[0].split("_")[1]);
				if (res.includes(dimId)) {
					return true;
				}
				var deleteDimension = $(".fields_list").find("[hierarchy='"+hierarchy+"']");
				if (deleteDimension.length > 0) {
					var level =  deleteDimension.find('a').attr('level');


					$.each(deleteDimension.find('li'), function(key, value) {
						if ($(value).hasClass('temphide')) {
							level = $(value).find('a').attr('level');
							parentThis.workspace.query.helper.removeParameter(hierarchy, level);
						}
					});
					parentThis.workspace.$el.find('.parameter_input').empty();

					parentThis.workspace.query.helper.removeLevel(hierarchy, level);
				}
				$(elem.parentElement).remove();
				parentThis.workspace.sync_query();
			});
			parentThis.workspace.query.run(true);
			Loader.off();
		});
	},
	...
	getItemsAxis:function (item) {
		switch (item.className){
			case "remove_row": return "ROWS"
		}
	},

	clear_axis: function (event) {
        event.preventDefault();
        var itemIndex = event.target.getAttribute("index");
		if (itemIndex) {
			this.workspace.query.helper.clearAxis(this.getItemsAxis(event.target),itemIndex);
		} else {
			if ($(event.target).siblings('.fields_list_body').parent().hasClass("details_fields")) {
				$('.details_fields .d_measure').remove();
				this.set_measures();
			} else {
    ...
    selections: function(event, ui) {
		...
		if (event.target.className === "remove_row") {
			return
		}

		if ($(event.target).hasClass('clear_dimension')) {
			return;
		}
    ...
    limit_axis: function(event) {
        ...
        // hardcode - локализованный рабочий title на разделах
        switch (target) {
          case 'Колонки':
            target = 'COLUMNS';
            break;
        ...
        $.contextMenu({
            ...
            build: function($trigger, e) {
                ...
                var cube = self.workspace.selected_cube;
                var items = {};
                var measures = query.model.queryModel.details.measures;
                ...
                var citems = {
                        ...
                        //  {
                        //         "customfilter": {name: "Настроить фильтр", i18n: false }, //hardcode. скрываем
                        ...
                        "limit" : {name: "Limit", i18n: true, items:
                        {
                                ...
                                // "customtop" : {name: "Настроить интервал значений", i18n: false }, //TODO: до реализации Requirement 125602
                        ...
                        "sort" : {name: "Sort", i18n: true, items:
                        {
                            ...
                            // "customsort" : { name: "Настроить сортировку", i18n: false }, //TODO: до реализации Requirement 125602
                        ...
                        "fold_totals": {name: "Totals", i18n: true, items:
                        {
                            ...
                            {
                                "show_totals_not": {name: "Очистить", i18n: false},
                                "show_totals_sum": {name: "Сумма", i18n: false},
                                "show_totals_min": {name: "Минимум", i18n: false},
                                "show_totals_max": {name: "Максимум", i18n: false},
                                "show_totals_avg": {name: "Среднее", i18n: false}
                            }}
                        }},
                        "cancel" : { name: "Cancel", i18n: true }
                ...
                _.each(selectedMeasures, function(measure) {
					measure.caption = measure.caption?measure.caption: measure.name;
                    var foldName = 'fold_' + measure.caption.replace(/\s/g, '_').toLowerCase();
                    var fold = {name: measure.caption, items: {}};

                    // Also applying a bold style to per measure's selected aggregation total

                    fold.items["show_totals_nil_" + measure.caption] = {name: self.formatAggregatorName("Очистить", "nil", a, measure), i18n: false};
                    fold.items["show_totals_sum_" + measure.caption] = {name: self.formatAggregatorName("Сумма",  "sum", a, measure), i18n: false};
                    fold.items["show_totals_min_" + measure.caption] = {name: self.formatAggregatorName("Минимум",  "min", a, measure), i18n: false};
                    fold.items["show_totals_max_" + measure.caption] = {name: self.formatAggregatorName("Максимум",  "max", a, measure), i18n: false};
                    fold.items["show_totals_avg_" + measure.caption] = {name: self.formatAggregatorName("Среднее",  "avg", a, measure), i18n: false};
