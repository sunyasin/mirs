            ...
            for (var m = 0; m < currentListNode.cells.length; m++) {
                ...
                if (bottom == lists.length - 1) {
                    ...
                    if (i == 0 && scanIndexes[i] == 0) {
                        ...
                        text += (wrapContent ? "<span class='i18n'>Итого</span>" :  "Итого");
                    }
                }
...
SaikuTableRenderer.prototype.internalRender = function(allData, options) {
    ...
    var type = allData.query.queryModel.details.axis;
	var location = allData.query.queryModel.details.location;
    ...
    for (var row = 0, rowLen = table.length; row < rowLen; row++) {
        ...
        if (typeof this._options.htmlObject === 'object' &&
        	...
        	if (type === "ROWS" && (auxColumnTitleTable === arrColumnTitleTable.length - 1)) {
        		colspanColumnTitleTable = 1;
			}
        ...
        if (type === "ROWS" && location == "BOTTOM") {
			for (var col = 0, colLen = table[row].length; col < colLen; col++) {
				if (table[row][col].value === "MeasuresLevel") {
					var firstTitle = $(rowContent).find("th")[0].title;
					var firstElem = table[row][0].value;
					for (var k=0; k < col - 1; k++) {
						table[row][k].value = table[row][k+1].value;
					}
					table[row][k].value = firstTitle;
					rowContent = rowContent.split(firstTitle).join(firstElem);
					break;
				}
			}
		}
        ...
        for (var col = 0, colLen = table[row].length; col < colLen; col++) {
            ...
            if (header.value === "MeasuresLevel") {
				if (header.type === "ROW_HEADER_HEADER") {
					lowestRowLvl = col;
				}
            	continue;
			}
    ...
    if (data && data.query) {
            ...
            if (m.aggregators) {
                for (var j = 0; j < m.aggregators.length; j++) {
                    ...
                    if (a.indexOf('_') > 0) {
                        ...
                        switch (aggregator) {
                          case "sum":
                            aggregatorBuffer = "Сумма";
                            break;
                          case "min":
                            aggregatorBuffer = "Минимум";
                            break;
                          case "max":
                            aggregatorBuffer = "Максимум";
                            break;
                          case "avg":
                            aggregatorBuffer = "Среднее";
                            break;
                        }
                        ...
                        if (aggregator !== 'nil' && aggregator !== 'not') {
                            ...
                            aggregatorBuffer = aggregatorBuffer.capitalizeFirstLetter();
                            if (!(axis in summaryData)) summaryData[axis] = [];
                            summaryData[axis].push(m.caption + ": " + aggregatorBuffer);
                        }
                    }
                }
            }
        ...
        if (hasSomethingToRender) {
            ...
            $.each(summaryData, function(key, aggregators) {
                ...
                summary += "<h3>" + keyBuffer.capitalizeFirstLetter();
