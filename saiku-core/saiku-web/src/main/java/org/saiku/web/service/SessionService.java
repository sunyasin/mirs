...
import org.saiku.database.dao.OlapUserDAO;
...
//				if (l.getLicense() instanceof SaikuLicense2) {
                    ...
                    if (SecurityContextHolder.getContext() != null
                        ...
                        if (authorisationPredicate.isAuthorised(auth)) {
							...
							database.addSchema(username, (String) sessionHolder.get(p).get("sessionid"));
...
//		return null;
	...
	public Map<String, Object> externalLogin(HttpServletRequest req, Integer userId, String fio) throws LicenseException {
		Object sl = null;
		String notice = null;
		HttpSession session = ((HttpServletRequest)req).getSession(true);
		session.getId();
		...
		String username = olapUserDAO.getExternalUserNameByIdAndFio(userId, fio);
		String password = "foms";
...
//		return null;
	...
	private void createSession(Authentication auth, String username, String password) {
		...
		if (isAnonOk && auth.isAuthenticated() && p != null && !sessionHolder.containsKey(p)) {
			...
			session.put("userId", olapUserDAO.getUserIdByUsername(username));
