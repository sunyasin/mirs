...
public class ExporterResource {
   ...
   */
	...
	public Response exportExcel(@QueryParam("file") String file, 
			@QueryParam("formatter") String formatter,@QueryParam("name") String name, @QueryParam("reportName") String reportName,
...
//			queryResource.execute(queryName, formatter, 0);
		  ...
		  if (StringUtils.isNotBlank(formatter)) {
			...
			return query2Resource.getQueryExcelExport(queryName, formatter, name, reportName);
		} catch (Exception e) {
...
//			QueryResult qr = query2Resource.execute(queryName, formatter, 0);
   ...
   */
	...
	*/
	...
	private String prepareSvg(String svg) {
  		String result = svg;
		if (StringUtils.isEmpty(svg)) return result;

		final String STRING_SEEK_BEGIN_FONT_SIZE = "font-size=\"";
		final String STRING_SEEK_END_FONT_SIZE = "px";
		result = findChangeAndReplaceValue(result, STRING_SEEK_BEGIN_FONT_SIZE, STRING_SEEK_END_FONT_SIZE, -2);

		final String STRING_SEEK_BEGIN_WIDTH = "height=\"";
		final String STRING_SEEK_END_WIDTH = "\"";
		result = findChangeAndReplaceValue(result, STRING_SEEK_BEGIN_WIDTH, STRING_SEEK_END_WIDTH, 80);

		return result;

	}

	private String findChangeAndReplaceValue(String svg, String stringBeginSeek, String stringEndSeek, Integer changeValue) {
		String result = svg;
		if (StringUtils.isEmpty(svg)) return result;

		Integer indexPosition = result.indexOf(stringBeginSeek) + stringBeginSeek.length();
		String currentString = result.substring(indexPosition, result.indexOf(stringEndSeek, indexPosition));
		try {
			Integer currentValue = Integer.valueOf(currentString);
			Integer newValue = currentValue + changeValue;
			result = result.replaceFirst(stringBeginSeek + currentString + stringEndSeek, stringBeginSeek + newValue + stringEndSeek);
