...
//			Date date = new Date();
//			document.set(new HeaderFooter(new Phrase("Saiku Export - " + dateFormat.format(date) + " Page: "), null));
	...
	private BaseColor color(BaseColor c, float percent){
	...
	private void populatePdf(Document doc, ArrayList<ReportData.Section> section, int dim, BaseColor color, float c) {
	  for (ReportData.Section aSection : section) {
		...
		Font myFont = FontFactory.getFont(FontFactory.HELVETICA, 8, BaseColor.WHITE);
		...
		if (aSection.getData() != null) {
		  ...
		  for (int t = 0; t < aSection.getData().length; t++) {
			for (int x = aSection.getData()[0].length
			  ...
			  int r = t % 2;
			  if (r != 0) {
				cell.setBackgroundColor(color(BaseColor.BLACK.BLACK, (float) 0.92));
			  }
