...
public class ExcelWorksheetBuilder {
    ...
    private List<List<String>> totalsSummary = new ArrayList<>();

    private List<List<String>> colsetHeaderAggr = new ArrayList<>();
    private List<List<String>> colsetBodyAggr = new ArrayList<>();

    private List<List<String>> rowsetBodyAggr = new ArrayList<>();
    ...
    private HashMap<String, CellStyle> totalsCSMap = new HashMap<>();
    private CellStyle numberCS;
    private HashMap<String, CellStyle> numberCSMap = new HashMap<>();
    ...
    private void init(CellDataSet table, List<ThinHierarchy> filters, ExcelBuilderOptions options) {
        ...
        if ("xls".equals(SaikuProperties.webExportExcelFormat) || options.asXls) {
        ...
        if (colScanTotals.keySet().size() > 0) {
            ...
            for (Integer colKey : colScanTotals.keySet()) {
                ...
                for (int x = 0; x < colAggregator.length; x++) {
                    ...
                    cell.setCellStyle(lighterHeaderCellCS);
                    if(table.getDetails().getAxis() != Axis.ROWS) {
                        cell.setCellValue(measure.getCaption() +  ":");
                        values.add(measure.getCaption() +  ":");
                    } else {
                        cell.setCellValue("По строке" +  ":");
                        values.add("По строке" +  ":");
                    }
...
//                    cell.setCellValue(agg.getClass().getSimpleName().substring(0, 3));
                    ...
                    cell.setCellValue(agg.getLabel());
                    values.add(agg.getLabel());
                }
                totalsSummary.add(values);
            }
        }
        ...
        if (rowScanTotals.keySet().size() > 0) {
            ...
            for (Integer rowKey : rowScanTotals.keySet()) {
                ...
                for (int x = 0; x < rowAggregator.length; x++) {
                    if(table.getDetails().getAxis() == Axis.ROWS && x > 0) {
                        break;
                    }
...
//                        cell.setCellValue(agg.getClass().getSimpleName().substring(0, 3));
    ...
    private void finalizeExcelSheet(int startRow) {
        ...
        Integer[] columnsWidthMax = getColumnsWidthMax();

        for (int i = 0; i < columnsWidthMax.length; i++) {
            int maxWidth = columnsWidthMax[i];
            if(maxWidth == 0) {
                workbookSheet.autoSizeColumn(i);
            } else {
                workbookSheet.setColumnWidth(i, maxWidth);
            }
        }
...
//        log.debug("Autosizing: " + (end - start) + "ms");
    ...
    private Integer[] getColumnsWidthMax() {

        int width = 0;
        int currentWidth = 0;
        for (List<String> values : colsetHeaderAggr) {
            currentWidth = values.size();
        ...
        Integer[] columnsWidthMax = new Integer[width];

        Integer[] columnsHeaderWidthMax = new Integer[width];
        for (int i = 0; i < columnsHeaderWidthMax.length; i++) {
            columnsHeaderWidthMax[i] = 0;
        }

        Integer[] columnsBodyWidthMax = new Integer[width];
        for (int i = 0; i < columnsBodyWidthMax.length; i++) {
            columnsBodyWidthMax[i] = 0;
        }

        Integer[] columnsCurrentWidth = new Integer[width];
        for (int i = 0; i < columnsCurrentWidth.length; i++) {
            columnsCurrentWidth[i] = 0;
        }

        String currentValue = null;
        int currentIdx = 0;

        for (AbstractBaseCell[] row : rowsetHeader) {
            for (int j = 0; j < row.length; j++) {
                currentValue = row[j].getFormattedValue();
                columnsCurrentWidth[j] = currentValue != null ? currentValue.length() : 0;
                if(columnsCurrentWidth[j] > columnsHeaderWidthMax[j]) {
                    columnsHeaderWidthMax[j] = columnsCurrentWidth[j];
                }
            }
        }
        ...
        for (List<String> values : colsetHeaderAggr) {
            for (int j = 0; j < values.size(); j++) {
                ...
                currentIdx = j + table.getWidth();
                columnsCurrentWidth[currentIdx] = currentValue != null ? currentValue.length() + 4 : 0;
                if(columnsCurrentWidth[currentIdx] > columnsHeaderWidthMax[currentIdx]) {
                    columnsHeaderWidthMax[currentIdx] = columnsCurrentWidth[currentIdx];
                }
            }
        }

        for (List<String> values : totalsSummary) {
            if(values.size() > 0) {
                currentIdx = 0;
                currentValue = values.get(currentIdx);
                columnsCurrentWidth[currentIdx] = currentValue != null ? currentValue.length() : 0;
                if(columnsCurrentWidth[currentIdx] > columnsHeaderWidthMax[currentIdx]) {
                    columnsHeaderWidthMax[currentIdx] = columnsCurrentWidth[currentIdx];
                }
            }
        ...
        for (AbstractBaseCell[] row : rowsetBody) {
            for (int j = 0; j < row.length; j++) {
                currentValue = row[j].getFormattedValue();
                columnsCurrentWidth[j] = currentValue != null ? currentValue.length() + 2 : 0;
                if(columnsCurrentWidth[j] > columnsBodyWidthMax[j]) {
                    columnsBodyWidthMax[j] = columnsCurrentWidth[j];
                }
            }
        }

        for (List<String> values : colsetBodyAggr) {
            for (int j = 0; j < values.size(); j++) {
                currentValue = values.get(j);
                currentIdx = j + table.getWidth();
                columnsCurrentWidth[currentIdx] = currentValue != null ? currentValue.length() + 2: 0;
                if(columnsCurrentWidth[currentIdx] > columnsBodyWidthMax[currentIdx]) {
                    columnsBodyWidthMax[currentIdx] = columnsCurrentWidth[currentIdx];
                }
            }
        }
        ...
        for (int j = 0; j < rowsetBodyAggr.size(); j++) {
            if(startColumnIndex >= j) {
                List<String> rowAggr = rowsetBodyAggr.get(j);
                for (String value : rowAggr) {
                    currentIdx = j;
                    currentValue = value;
                    columnsCurrentWidth[currentIdx] = currentValue != null ? currentValue.length() + 2 : 0;
                    if(columnsCurrentWidth[currentIdx] > columnsBodyWidthMax[currentIdx]) {
        ...
        int widthPerSymbol = 280;
        int maxLength = 50;
        for (int i = 0; i < columnsBodyWidthMax.length; i++) {
            if(columnsHeaderWidthMax[i] < columnsBodyWidthMax[i]) {
                if(columnsBodyWidthMax[i] <= maxLength) {
                    // иногда года вплотную приоегают друг к другу
                    if(columnsBodyWidthMax[i] <= 5) {
                        columnsWidthMax[i] = 6 * widthPerSymbol;
                    } else {
                        // пытаемся выровнять по ширине
                        columnsWidthMax[i] = columnsBodyWidthMax[i] * widthPerSymbol;
                    }
                } else {
                    // фиксированное значение
                    columnsWidthMax[i] = maxLength * widthPerSymbol;
                }
            } else {
    ...
    private void initSummarySheet() {
        ...
        // Main Workbook Sheet
        Sheet summarySheet = excelWorkbook.createSheet("Сводная страница");
        ...
        cell.setCellValue("Дата и время формирования отчета: " + todayDate);
        ...
        cell.setCellValue("Измерение");
        cell = sheetRow.createCell(1);
        cell.setCellValue("Уровень");
        cell = sheetRow.createCell(2);
        cell.setCellValue("Применен фильтр");
        row++;
...
//        cell.setCellValue(SaikuProperties.webExportExcelPoweredBy);
    ...
    private int summarySheet(int startingRow) {

        int row = startingRow;

        Row sheetRow = workbookSheet.createRow((int) row);
        Cell cell = sheetRow.createCell(0);
        String todayDate = (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
        cell.setCellValue("Дата и время формирования отчета: " + todayDate);

        sheetRow = workbookSheet.createRow((int) row);
        cell = sheetRow.createCell(0);
        if(this.options.reportName != null) {
            cell.setCellValue(this.options.reportName);
        } else {
            cell.setCellValue("Анализ показателей");
        }
        ...
        CellStyle headerCellCS = excelWorkbook.createCellStyle();
        headerCellCS.setFont(headerFont);
        headerCellCS.setAlignment(CellStyle.ALIGN_LEFT);

        cell.setCellStyle(headerCellCS);

        row = row + 1;

        if (queryFilters != null && !queryFilters.isEmpty()) {
            Font filterFont = excelWorkbook.createFont();
            filterFont.setFontHeightInPoints((short) BASIC_SHEET_FONT_SIZE);
            filterFont.setFontName(BASIC_SHEET_FONT_FAMILY);
            filterFont.setBoldweight(Font.BOLDWEIGHT_BOLD);

            CellStyle filterCellCS = excelWorkbook.createCellStyle();
            filterCellCS.setFont(filterFont);
            filterCellCS.setAlignment(CellStyle.ALIGN_LEFT);

            row = row + 1;

            sheetRow = workbookSheet.createRow((int) row);
            cell = sheetRow.createCell(0);
            cell.setCellStyle(filterCellCS);
            cell.setCellValue("Измерение");
            cell = sheetRow.createCell(1);
            cell.setCellStyle(filterCellCS);
            cell.setCellValue("Уровень");
            ...
            for (ThinHierarchy item : queryFilters) {
                for (ThinLevel s : item.getLevels().values()) {
                    for (ThinMember i : s.getSelection().getMembers()) {
                        sheetRow = workbookSheet.createRow((short) row);
                        cell = sheetRow.createCell(0);
                        cell.setCellValue(item.getCaption());
                        cell = sheetRow.createCell(1);
                        cell.setCellValue(s.getCaption());
...
//            cell = sheetRow.createCell(0);
//            cell.setCellValue("Excel sheet is truncated, only contains " + maxColumns + " columns of " + (columnCount));
//            workbookSheet.addMergedRegion(new CellRangeAddress(row, row, 0, 10));
//            row++;
//        }
//
//        if ((headerLength + rowLength) > maxRows) {
...
//        }
    ...
    private TotalAggregator[][] clearTotalAggregator (TotalAggregator[][] tg) {
        List<TotalAggregator[]> totalAggregatorList = new ArrayList<>();
        for (int i = 0; i < tg.length; i++) {
            TotalAggregator[] aggregators = tg[i];
            boolean isAllNotBlank = false;
            for (TotalAggregator totalAggregator : aggregators) {
                if(!(totalAggregator instanceof BlankAggregator)) {
                    isAllNotBlank = true;
                    break;
                }
            }
            ...
            if(isAllNotBlank) {
                totalAggregatorList.add(aggregators);
            }
        }

        TotalAggregator[][] result = new TotalAggregator[totalAggregatorList.size()][];
        for (int i = 0; i < totalAggregatorList.size(); i++) {
            result[i] = totalAggregatorList.get(i);
        }
        return result;
    }
    ...
    private int setRowTotalAggregationCell(Map<Integer, TotalAggregator[][]> scanTotals, int startIndex, int subIndex, boolean grandTotal) {
        if (!scanTotals.isEmpty()) {
            ...
            TotalAggregator[][] aggregatorsTable = scanTotals.get(subIndex + rowsetHeader.length);
            if (aggregatorsTable != null) {
                ...
                if(table.getDetails().getAxis() != Axis.ROWS) {
                    List<String> rowAggr = new ArrayList<>();
                    rowsetBodyAggr.add(rowAggr);
                    ...
                    if (grandTotal) {
                        ...
                        workbookSheet.addMergedRegion(new CellRangeAddress(row + 1, row + 1, 0, startColumnIndex));
                    }
                    ...
                    for (TotalAggregator[] aggregators : aggregatorsTable) {
                        ...
                        for (TotalAggregator aggregator : aggregators) {
                            ...
                            try {
                                String formatString = FormatUtil.getFormatString(aggregator.getFormat().getFormatString());
                                if (formatString != null && formatString.length()>0) {
                                    CellStyle totalsCSClone = totalsCSMap.get(formatString);
                                    if (totalsCSClone == null) {
                                        DataFormat fmt = excelWorkbook.createDataFormat();
                                        short dataFormat = fmt.getFormat(formatString);
                                        totalsCSClone = excelWorkbook.createCellStyle();
                                        totalsCSClone.cloneStyleFrom(totalsCS);
                                        totalsCSClone.setDataFormat(dataFormat);
                                        totalsCSMap.put(formatString, totalsCSClone);
                                    }
                                    cell.setCellStyle(totalsCSClone);
                                }
                            } catch (Exception ex) {
                    ...
                    for (int i = 0; i < aggregatorsTable.length; i++) {
                        List<String> rowAggr = new ArrayList<>();
                        rowsetBodyAggr.add(rowAggr);
                        Row sheetRow = workbookSheet.createRow(row + 1);

                        String captionMeasure = table.getSelectedMeasures()[i].getCaption();

                        TotalAggregator[] aggregators = aggregatorsTable[i];
                        String label = aggregators[0].getLabel();

                        for (int j = 0; j <= startColumnIndex; j++) {
                            Cell cell = sheetRow.createCell(j);
                            cell.setCellValue(captionMeasure + " " + label);
                            cell.setCellStyle(basicCS);
                        }
                        workbookSheet.addMergedRegion(new CellRangeAddress(row + 1, row + 1, 0, startColumnIndex));
        ...
        if (!scanTotals.isEmpty()) {
            ...
            if (aggregatorsTable != null) {
                if (setValue) {
                    ...
                    for (TotalAggregator[] aggregators : aggregatorsTable) {
                        aggrValues.add(aggregators[x].getFormattedValue());
                        Cell cell = sheetRow.createCell(column);
                        Double value = aggregators[x].getValue();
                        cell.setCellValue(value != null ? value : 0);
                        cell.setCellStyle(totalsCS);
                        try {
                            String formatString = FormatUtil.getFormatString(aggregators[x].getFormat().getFormatString());
                            if (formatString != null && formatString.length()>0) {
                                CellStyle totalsCSClone = totalsCSMap.get(formatString);
                                if (totalsCSClone == null) {
                                    DataFormat fmt = excelWorkbook.createDataFormat();
    ...
    private void applyCellFormatting(Cell cell, DataCell dataCell) {
        ...
        */
        String formatString = dataCell.getFormatString();
        CellStyle numberCSClone = numberCSMap.get(formatString);
        if (numberCSClone == null) {
            numberCSClone = excelWorkbook.createCellStyle();
            ...
            numberCSMap.put(formatString, numberCSClone);
        }
    ...
    private int buildExcelTableHeader(int startRow) {
        ...
        // добавляем дополнительные поля в заголовк
        AbstractBaseCell[] aggrRowSetHeader = null;
        int currentAggrHeaderIdx = rowsetHeader.length -1;
        if (!colScanTotals.isEmpty()) {

            aggrRowSetHeader = rowsetHeader[currentAggrHeaderIdx];
            for (int i = 0; i < aggrRowSetHeader.length; i++) {
                TotalAggregator[][] aggregatorsTable = colScanTotals.get(i + 1);
                if (aggregatorsTable != null) {
                    int diff = aggregatorsTable.length;
                    int oldSize = aggrRowSetHeader.length;
                    int newSize = oldSize + diff;
                    AbstractBaseCell[] newLastRowSetHeader = Arrays.copyOf(aggrRowSetHeader,  newSize);
                    List<String> aggrValues = new ArrayList<>();

                    for (int j = 0; j < aggregatorsTable.length; j++) {
                        String captionMeasure = table.getSelectedMeasures()[j].getCaption();
                        int currentNewIdx = aggrRowSetHeader.length + j;
                        String aggrName = null;
                        if(table.getDetails().getAxis() != Axis.ROWS) {
                            aggrName = captionMeasure + " " + aggregatorsTable[j][0].getLabel();//aggrRowSetHeader[currentNewIdx - diff].getFormattedValue() + " " + aggregatorsTable[j][0].getLabel();
                        } else {
                            aggrName = aggregatorsTable[j][0].getLabel();
                        }

                        MemberCell cell = new MemberCell();
                        cell.setFormattedValue(aggrName);
                        newLastRowSetHeader[currentNewIdx] = cell;
                        // если значении не будет, нужна другая логика
                        aggrValues.add(aggrName);
                    }
                    colsetHeaderAggr.add(aggrValues);
                    rowsetHeader[currentAggrHeaderIdx] = newLastRowSetHeader;
                    break;
                }
            }
        }
        ...
        for (x = 0; x < rowsetHeader.length; x++) {
            ...
            for (y = 0; y < maxColumns && y < rowsetHeader[x].length; y++) {
                ...
                if (currentHeader != null) {
                    ...
                    if(currentHeader.equals("MeasuresLevel")) {
                        currentHeader = "Показатель";
                    }
                    if(nextHeader.equals("MeasuresLevel")) {
                        nextHeader = "Показатель";
                    }
                    ...
                    fillHeaderCell(sheetRow, EMPTY_STRING, column);
                    startSameFromPos++;
                }

                //Set sub total column space
                int nextColumn = column + 1; //setColTotalAggregationCell(colScanTotals, sheetRow, x, column, false, false);
        ...
        // возвращаем на место
        if(aggrRowSetHeader != null) {
            rowsetHeader[currentAggrHeaderIdx] = aggrRowSetHeader;
        }
        return x + startRow;
    }
